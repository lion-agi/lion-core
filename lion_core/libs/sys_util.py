...
class SysUtil:
    ...
    @staticmethod
    def insert_random_hyphens(s: str, n_hyphens: int = 1, keep_n_front: int = 1) -> str:
        if len(s) <= 6:
            raise ValueError("String length must be greater than 6.")
        
        # Exclude the first n characters from being modified
        prefix = s[:keep_n_front]
        modifiable_part = s[keep_n_front:]
        
        # Determine positions to insert the hyphens
        positions = random.sample(range(len(modifiable_part)), n_hyphens)
        positions.sort()
